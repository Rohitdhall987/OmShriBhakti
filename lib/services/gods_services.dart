...
class GodsService {
  ...
  Future<GodDataModel> fetchGodsPlaylist(int id) async {

    try {
      // Construct the API URL
      final apiUrl = '${_url}singleGod/$id';
      ...
      if (response.statusCode == 200) {
        ...
        // Parse the JSON into a Videos object
        final godData = GodDataModel.fromJson(data);
        return godData; // Return the list of Video objects
      } else {
        throw "API Error: ${response.statusCode} - ${response.reasonPhrase}";
      }
    } catch (e) {
      throw "Unable to fetch Gods data: $e";
    }
  }
}
